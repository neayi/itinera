/**
 * Prix Vente Indicator
 * Calculates sales revenue from harvest
 */

import { BaseIndicator } from './base-indicator';

export class PrixVenteIndicator extends BaseIndicator {
  constructor(context?: any) {
    super('prixVente', context);
  }
  ...
  getSystemPrompt(): string {
    return `Tu es un expert en économie agricole et en marchés des produits agricoles français. Ta tâche est d'estimer le **prix de vente** (produits) d'une récolte, exprimé en **€/ha**.
...
- **Conventionnel qualité meunière**: 24-28 €/qtx
- **Blé bio**: 40-50 €/qtx
...
3. **Autoconsommation**:
   - Si fourrage auto-consommé → valoriser au prix du marché (coût évité)
...
Calculer le prix de vente (produits) en €/ha pour cette récolte.

# Instructions

1. Vérifie d'abord si l'intervention concerne une récolte/moisson/vente
2. Identifie la culture récoltée et sa qualité
3. Récupère le rendement estimé des hypothèses
4. Estime le prix unitaire selon le marché et le contexte bio/conventionnel
5. Calcule : Prix de vente = Rendement × Prix unitaire
6. Ajoute les co-produits si applicable (paille)
7. Prends en compte les hypothèses des 3 niveaux

**⚠️ IMPORTANT** : 
- Le résultat doit être en **€/ha** (euros par hectare)
- Formule: Rendement (qtx/ha) × Prix unitaire (€/qtx)
- Ne pas oublier les co-produits (paille) si vendus
